...
def main(data_dir):
    ...
    for ep in range(start_epoch, target_epoch+1):
        bar = tqdm(wlp300_dataloader)
        Loss_list, Stat_list = [], []
        for i, sample in enumerate(bar):
            uv_map, origin = sample['uv_map'].cuda(), sample['origin'].cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_dir", default='./dataset/300WLP',help="specify input directory.")
    args = parser.parse_args()
    '''
    pwd = os.getcwd()
    home_path = os.path.abspath(os.path.dirname(pwd) + os.path.sep + "../..")
    save_dir = os.path.join('../', home_path, '/home/ubuntu/hdd1/lxc/300WLP/')
    '''
    main(args.train_dir)
